#!/usr/bin/env bash
# 8-GPU PI_BEHAVIOR 100-task generalist (task embeddings + System 2):
# 60% 2026-challenge-demos (all 100 tasks) + 40% Comet RFT, 72h wall clock.
set -euo pipefail
...
CONFIG_NAME="pi_behavior_2026_all100_demo0_6_comet0_4"
EXP_NAME="${OPENPI_EXP_NAME:-pi_behavior_2026_all100_demo0_6_comet0_4_$(date +%Y%m%d_%H%M%S)}"
...
export OPENPI_DATALOADER_PREFETCH_FACTOR="${OPENPI_DATALOADER_PREFETCH_FACTOR:-8}"
export OPENPI_BATCH_PREFETCH="${OPENPI_BATCH_PREFETCH:-8}"
...
if [ ! -f "${ASSETS_DIR}/norm_stats.json" ]; then
  echo "Computing 100-task norm stats + correlation matrix..."
...
if [ ! -f "${ASSETS_DIR}/fast_tokenizer/processor_config.json" ] && [ ! -f "${ASSETS_DIR}/fast_tokenizer/tokenizer.json" ]; then
  echo "Training FAST tokenizer on 100-task mix..."
...
echo "Starting 8-GPU 100-task training exp=${EXP_NAME} for up to ${TRAIN_WALL_CLOCK}"
